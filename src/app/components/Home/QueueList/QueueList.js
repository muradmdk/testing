...
function TrackItem({
  ...
  useEffect(() => {
    const checkOverflow = () => {
      const h4Element = h4Ref.current;
      if (h4Element) {
        setIsOverflow(h4Element.scrollWidth > h4Element.clientWidth);
      }
    };
    ...
    // Delay the check to ensure layout is complete
    setTimeout(checkOverflow, 500); // Delay the overflow check
    return () => {
                ...
                <div style={{ display: "inline-grid" }}>
                  {isOverflow ? (
                    <h4 className="mb-1">
                      ...
                      <marquee behavior="smooth" direction="left" scrollamount="2" pa>{item.trackName}</marquee>
                    </h4>
                  ) : (
                    <h4
                      ref={h4Ref}
                      className={`mb-1 ${
                        isOverflow ? "overflowText" : "nooverflow"
                      }`}
                    >
                      {item.trackName}
                    </h4>
