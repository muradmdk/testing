...
function CreateSong() {
  ...
  const { id } = useParams();
  const { data: trackData, isLoading: isTrackLoading } =
    useGetSingleTrackQuery(id);
  ...
  useEffect(() => {
    if (trackData?.result === "success") {
      const track = trackData?.data;
      setInitialValues({
        song_name: track.song_name || "",
        album_id: track.album_id || "",
        song_description: track.song_description || "",
        subscription_level_id: track.subscription_level_id || "",
        publish_date: track.publish_date?.split("T")[0] || "",
        song_isrc: track.song_isrc || "",
        release_date: track.release_date?.split("T")[0] || "",
        ...
        cover_picture: track.cover_file || null,
        permalink: track.permalink ? track.permalink.split("/").pop() : "",
        track_file: track.track_file || null,
        genre_id:
          track.genre_id?.map((genre) => ({
            value: genre,
            label: genres?.find((g) => g.id === genre)?.name || genre,
          })) || [],
      });

      // Set cover image and track file URLs
      if (track.cover_file) {
        setCoverImage(
          `${process.env.NEXT_PUBLIC_IMG_BASE_URL}/${track.cover_file}`
        );
      }
      if (track.track_file) {
        setTrackFile(
          `${process.env.NEXT_PUBLIC_IMG_BASE_URL}/${track.track_file}`
        );
      }
    }
  }, [trackData, genres]);
